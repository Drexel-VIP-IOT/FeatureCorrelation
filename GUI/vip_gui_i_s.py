...
class App(QMainWindow):
    ...
    def on_click(self):
        if self.btn_press_cnt == 0:
            ...
            ae_name = self.textbox.text()
            ir_name = self.textbox_1.text()
            dic_name = self.textbox_2.text()
            coll_name = self.textbox_3.text()
            doc_name = self.textbox_4.text()

            self.run_dataloader(coll_name, doc_name)
            self.run_ae(ae_name)
    ...
    def run_dataloader(self, coll, doc):
        client = paramiko.client.SSHClient()
        client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        client.load_system_host_keys()
        client.connect('192.168.1.15', username='pi', password='Memmech1')
        client.exec_command('cd /home/pi/Desktop/raspcode/DEV && sudo python3 mongodb_json_upload.py {} {}'.format(coll, doc))

    def run_ae(self, ae):
        client = paramiko.client.SSHClient()
        client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        client.load_system_host_keys()
        client.connect('192.168.1.24', username='pi', password='Memmech1')
        client.exec_command('cd /home/pi/Desktop/raspcode/FeatureCorrelation/ae && sudo python3 chunk_it.py {}'.format(ae))
